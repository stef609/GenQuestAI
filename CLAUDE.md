...
GenQuestAI is an application for letting LLMs play text adventure games like Zork. The LLM takes the place of a human player, interacting with the game's text interface and attempting to solve puzzles and maximize its score.
...
# Sync dependencies after manual pyproject.toml changes
uv sync
...
- `zork.md` - LLM prompt guide for playing Zork
- `zork.help` - Zork game documentation (commands, mechanics, parser rules)
...
The project interacts with Zork, a classic text adventure. Important concepts:
...
- Commands include: `NORTH`/`N`, `SOUTH`/`S`, `TAKE`, `DROP`, `INVENTORY`/`I`, `LOOK`/`L`, `SAVE`, `RESTORE`, `SCORE`, `QUIT`/`Q`

**Game Systems:**
- **Containers:** Can be open/closed, may be transparent, have capacity limits
- **Combat:** Fighting strength decreases when injured/killed, regenerates over time
- **Scoring:** Points collected through treasures and puzzle solving

The `zork.help` and `zork.md` files contain complete reference material for the game mechanics and an LLM prompt template.
